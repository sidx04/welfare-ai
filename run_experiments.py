...
print(
    f"Starting sweep: {len(SCHEME_IDS)} schemes x {len(PROFILES)} profiles = {total} experiments\n"
)
...
for scheme_id in SCHEME_IDS:
    ...
    for label, user_profile in PROFILES:
        ...
        baseline_output = run_baseline(
            llm, scheme_id, scheme["scheme_name"], user_profile
        )
...
print(
    f"Done. {total} experiments logged -> logs/experiments.jsonl  ({overall_elapsed:.1f}s total)"
)
